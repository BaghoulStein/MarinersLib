...
//TODO: change harcoded variables to be in the format of constants
...
public class JoystickAxis extends Trigger {
    ...
    {
        if (c.getRawAxis(axis) > 0.99)
            return State.fullyHeld;
        else if (c.getRawAxis(axis) > 0.1)
            return State.held;
        return State.letGo;
    }
    ...
    public void onFullyHeld(Command command)
    {
        CommandScheduler.getInstance().getDefaultButtonLoop().bind(
            new Runnable() {
                ...
                @Override
                public void run() {
                boolean pressed = c.getRawAxis(axis) > 0.99;
